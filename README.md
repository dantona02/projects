...
The four video files provided in this repository show the simulation of three bodies with equal mass, i.e. $m_1=m_2=m_3$ of the sun $m_{\text{sun}}=1.98892\cdot10^{30}\ \text{kg}$ and an initial distance of one astronomical unit $\text{(}L=1.495978707\cdot10^{11}\ \text{m}\text{)}$ to each other, but different $v_1$ and $v_2$.
